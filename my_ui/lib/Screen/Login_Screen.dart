...
class _LoginState extends State<Login_Screen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Main_body(context),
    );
  }

  // 이 아래의 과정은 긴 요소를 method로 추출하였다.
  // 현재는 class 내부에 있기에 다른 클래스로 분할하진 않았다.
  // 반환은 Widget으로 지정하였고, ScaffoldMessenger를 위해 context를 매개변수로 받아오게 된다. <자동으로 지정하자!>
  // 이때 우리가 _Main_body로서 지정하면 이는 private가 된다. <같은 파일에서만 접근 가능>
  Widget Main_body(BuildContext context) {
